...
class RunSpec:
    ...
    def validate(self) -> None:
        ...
        if self.resume or self.operation == "validation":
...
def route_for_spec(spec: RunSpec) -> str:
    if spec.dry_run or spec.operation in {"cpu-test", "validation"}:
        return "cpu"
...
def build_cli_args(spec: RunSpec, *, mount: Path = RESULTS_MOUNT) -> list[str]:
    ...
    return ["dlmrel", "validate", "--run-dir", run_dir]
    ...
    def _remote_worker(spec_json: str, lock_token: str) -> str:
        ...
        try:
            with tempfile.TemporaryDirectory(prefix="dlmrel-source-") as temporary:
                ...
                # Generated UD manifests are intentionally not committed. A fresh Modal
                # checkout therefore prepares only the selected, revision-pinned dataset
                # before a real experiment. The existing CLI verifies the configured
                # source revision and checksums and stops on any mismatch.
                if spec.operation == "experiment-run" and not spec.dry_run:
                    subprocess.run(
                        ["dlmrel", "prepare", "--dataset", spec.dataset_config],
                        cwd=repository,
                        env=_sanitized_environment(CACHE_MOUNT),
                        shell=False,
                        check=True,
                        timeout=min(1_800, spec.timeout_seconds),
                    )
