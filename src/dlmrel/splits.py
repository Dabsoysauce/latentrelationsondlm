...
def load_tokenizer(name: str):
    ...
    Dream ships a custom tokenizer class, so a plain load raises with an
    instruction to pass `trust_remote_code=True`. Running that code is inherent
    to using Dream at all; selecting a config that names such a model is the
    opt-in.
    """
    ...
    except Exception as exc:  # noqa: BLE001 - transformers raises several types
        print(
