...
def manual_token_offsets(
    ...
    """Reconstruct per-token character offsets for a slow tokenizer.

    Same left-to-right cursor scan as `find_char_spans`: decode each token and
    locate it in the text at or after the cursor. A token that cannot be placed
    gets a zero-width span, which no word overlaps, so a failure here drops the
    word from the alignment (and, under `require_full_alignment`, the sentence
    from the pool) rather than aligning it to the wrong position.
    """
...
def token_offsets(tokenizer, text: str) -> list[tuple[int, int]]:
    ...
    Fast tokenizers provide `return_offsets_mapping`; slow ones (Dream's
    remote-code tokenizer, for example) raise instead, so the capability is
    probed once and remembered on the tokenizer object.
    """
    ...
    if fast is None:
        try:
            ...
            fast = True
        except Exception:  # noqa: BLE001 - slow tokenizers raise various types
            fast = False
