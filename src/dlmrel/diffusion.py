...
def forward_with_attentions(model, input_ids, attention_mask):
    ...
    Mask-free models (Dream) are natively bidirectional: they take the raw ids
    with no attention mask, and their adapter owns the forward call so this
    module never has to import the DiffuLLaMA patch for them.
    """
...
def attentions_at_time(
    ...
    # The DiffuLLaMA families need the repo's fully-bidirectional anneal mask;
    # importing that module is what applies the transformers-4.44 patch, so it
