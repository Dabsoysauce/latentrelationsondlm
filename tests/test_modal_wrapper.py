...
def test_time_curve_requires_isolated_lock_and_cpu_finalizer_never_names_a_model():
    ...
    finalizer = make_spec(
        ...
        source_result_identity="staged/dream-copy",
    )
    assert build_cli_args(finalizer) == [
        "dlmrel",
        "finalize-head-search",
        "--run-dir",
        str(finalizer.run_directory()),
    ]
    assert "dream_7b.yaml" not in build_cli_args(finalizer)


def test_dream_recovery_routes_only_the_missing_all_head_test_stage():
    recovery = make_spec(
        ...
        expected_scientific_config_hash=SCIENCE_HASH,
        expected_manifest_hashes=MANIFESTS,
        source_result_identity="staged/dream-english-head-3seed-v1",
    )
    assert route_for_spec(recovery) == "dream"
    args = build_cli_args(recovery)
    assert args[1] == "recover-head-search-test-grid"
    assert "run" not in args[1:2]
    assert "prepare" not in args
...
def test_recovery_copies_source_and_validates_identity_without_mutating_source(tmp_path):
    results = tmp_path / "results"
    source = results / "staged" / "dream-copy"
    source.mkdir(parents=True)
    (source / "run_metadata.json").write_text(
        json.dumps({"scientific_config_hash": SCIENCE_HASH}), encoding="utf-8"
    )
    (source / "manifest_refs.json").write_text(json.dumps(MANIFESTS), encoding="utf-8")
    sentinel = source / "select_instances.parquet"
    ...
    def runner(args, **kwargs):
        assert args[1] == "recover-head-search-test-grid"
        assert spec.run_directory(results).is_dir()
        return ProcessOutcome(0, "grid complete", "")
    ...
    assert spec.run_directory(results) != source


def test_recovery_accepts_only_the_authoritative_legacy_hash_migration(tmp_path):
    results = tmp_path / "results"
    source = results / "staged" / "dream-copy"
    source.mkdir(parents=True)
    (source / "run_metadata.json").write_text(
    ...
    def authoritative_migration(args, **kwargs):
        metadata_path = spec.run_directory(results) / "run_metadata.json"
        metadata_path.write_text(
            json.dumps(
                {
                    "scientific_config_hash": normalized_hash,
                    "legacy_scientific_config_hash": SCIENCE_HASH,
                }
            ),
            encoding="utf-8",
        )
        return ProcessOutcome(0, "migration complete", "")
    ...
    result = execute_spec(
        ...
        cache_mount=tmp_path / "cache",
        logs_mount=tmp_path / "logs",
        process_runner=authoritative_migration,
    )
    assert result.status == "success"
    assert result.scientific_config_hash == normalized_hash

    incompatible = replace(spec, attempt=2, source_result_identity=result.result_directory)

    def incompatible_change(args, **kwargs):
        metadata_path = incompatible.run_directory(results) / "run_metadata.json"
        metadata_path.write_text(
            json.dumps({"scientific_config_hash": "f" * 64}), encoding="utf-8"
        )
        return ProcessOutcome(0, "incompatible", "")

    rejected = execute_spec(
        incompatible,
        repository=Path.cwd(),
        results_mount=results,
        cache_mount=tmp_path / "cache",
        logs_mount=tmp_path / "logs",
        process_runner=incompatible_change,
    )
    assert rejected.status == "failed"
    assert rejected.failure_category == "scientific_configuration_mismatch"
...
def test_run_identity_is_attempt_and_commit_independent_for_concurrency():
    ...
    assert first.run_identity == finalizer.run_identity
    assert first.results_root() != finalizer.results_root()
